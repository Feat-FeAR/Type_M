...
for i=1:3
  ...
  % circular mean angle
  stats(i,1) = circ_mean(ori,spk);
  
  % circular variance
  stats(i,2) = circ_var(ori,spk,dori);
  
  % circular standard deviation
  [stats(i,3) stats(i,4)] = circ_std(ori,spk,dori);
  
  % circular skewness
  [stats(i,5) stats(i,6)] = circ_skewness(ori,spk); 
  
  % circular skewness
  [stats(i,7) stats(i,8)] = circ_kurtosis(ori,spk);
  
  % confidence limits on mean angle
  t = circ_confmean(ori,[],spk,dori);
